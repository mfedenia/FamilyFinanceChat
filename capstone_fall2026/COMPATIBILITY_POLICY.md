...
| C-10 | A provider API key was committed and stayed live on the public default branch until 2026-08-19 (the scrub sat on an unmerged branch). The folder is now deleted, but **provider-side revocation is still outstanding** — the key remains in git history and in the fork | git history under `scoring_page/` | **High** | Revoke the key at the provider — nothing consumed it, so revoking beats rotating. Add `gitleaks` to CI so it cannot recur |
