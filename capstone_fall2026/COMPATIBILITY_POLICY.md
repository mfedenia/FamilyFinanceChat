...
| C-9 | ~~Scoring rubric and ABI formulas implemented twice — JavaScript and Python~~ **Closed 2026-08-19:** `scoring_page/` deleted; `grading_feature/backend/scoring_service.py` is canonical. Unit tests pinning the rubric arithmetic are still owed (task A21) | `grading_feature/backend/scoring_service.py` | — | Done, apart from the tests |
| C-10 | ~~A provider API key was committed and stayed live on the public default branch until 2026-08-19.~~ **Closed 2026-08-19:** the key belongs to a third party, so **nobody here can revoke it**; nothing in the platform ever used it. `scoring_page/` is deleted; the value survives in git history and in a fork regardless | git history under `scoring_page/` | — | No action available. Land `gitleaks` in CI (A4) so a key that *is* ours cannot leak the same way. See `docs/memory/leaked-key-not-ours-to-revoke.md` |
