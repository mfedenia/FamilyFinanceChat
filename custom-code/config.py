...
def run_migrations():
    ...
    try:
        ...
        err = str(e)
        if any(x in err for x in ["duplicate column", "already exists", "no such revision"]):
            log.warning("Migration skipped (already applied): " + err)
        else:
            log.exception("Error running migrations: " + err)
